...
TEST_CASE("Case 3: Equality.") {
    ...
    Fraction f5 (-4, -4);
    CHECK_EQ(f5, 1);
    // ---- Testing: Creating a fraction where both the denominator is 1..
    // ---- Expecting: Should be equal to the numerator..
    Fraction f6 (5, 1);
    CHECK_EQ(f6, 5);
}

TEST_CASE("Case 4: Zero.") {
    // ---- Testing: Creating a fraction with 0 as numerator and adding 0 to it.
    // ---- Expecting: Should be equal to the number 0.
    Fraction f1 (0, 2);
    CHECK_EQ(f1 + 0, 0);
    // ---- Testing: Creating a fraction where both numerator and denominator are negative.
